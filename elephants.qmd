...
@Bloomfield:2021va claims to use regression discontinuity design (RDD), but arguably does not.^[To be fair, many papers in accounting research that claim to use RDD generally do not implement RDD according to the conventions of the literature in which it was developed [e.g., @Lee:2010wu].
While @Bloomfield:2021va might claim to be implementing "RDD" as understood by accounting researchers, accounting researchers shoudl explain why they deviate from standard approaches and validate the ones they use.
While @Bloomfield:2021va cites @Iliev:2010ic, his approach differs from @Iliev:2010ic in a number of ways and his setting is effectively quite different.
See @far_book_24 for details of RDD as used in accounting research.]
Replace the difference-in-difference analysis of @Bloomfield:2021va with a standard fuzzy RDD analysis and results vanish.^[See @far_book_24 for details.
The setting of @Iliev:2010ic and @Bloomfield:2021va is a "fuzzy RDD" setting requiring use of instrumental variable (IV). @Iliev:2010ic presents IV estimates, but @Bloomfield:2021va does not.
As discussed in @far_book_24, the @Iliev:2010ic instrument is surely too weak in the @Bloomfield:2021va setting to yield reliable inferences.]
