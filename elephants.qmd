...
To be sure, this is a very big elephant: I conjecture that p-hacking is the dominant mode of research in academic accounting in 2022 and below provide (admittedly circumstantial) evidence consistent with this conjecture.

That the basic concern of @Ohlson:2022aa is with p-hacking is clearest with the last of his five elephants: "Issues related to 'screen-picking' and 'data-snooping'", as terms like "data-snooping" are simply synonyms of p-hacking.^[I discuss below in @sec-other-elephants how the remaining four elephants relate to p-hacking.]
...
I agree with @Ohlson:2022aa that we need to confront this "elephant in the room" and make a number of proposals for how we might do so.
...
The contention of @Ohlson:2022aa that raising issues related to p-hacking is taboo in empirical accounting seminars seems very plausible.
...
A more brazen form of p-hacking is trawling through a data set until correlations are found, at which point the challenge is to devise an "interesting" causal story to go with it.^[See my [LinkedIn post](https://www.linkedin.com/pulse/p-hacking-accounting-research-ian-gow) on this topic.]
...
Sherlock Holmes claims "it is a capital mistake to theorize before you have all the evidence."^[From [*A Study in Scarlet*](https://www.gutenberg.org/files/244/244-h/244-h.htm).]
...
While it is difficult to rule out this explanation completely, I believe a significant recent project suggests this is not a complete explanation.
...
As discussed [in the book](https://iangow.github.io/far_book/introduction.html#features-of-this-course), a core element is material focused on data analysis skills and we have included many replication analyses to support this.
...
[We find](https://iangow.github.io/far_book/beaver68.html) that key results of @Beaver:1968vf hold in any year we look at.^[@Bamber:2000wv raise concerns about the reproducibility of the results in @Beaver:1968vf, but these concerns that do not appear to hold in years after @Beaver:1968vf was published.]
...
If authors do make code and data available, replication is often difficult.
...
Replace simple difference-in-difference analysis of @Bloomfield:2021va's with a proper RDD analysis and results vanish.
...
Direct evidence of p-hacking is general impossible to come by and p-hacking is something more easily inferred for an area of research than for a single paper.]
If other papers where authors do not provide detailed code and data are similarly fragile, then we would not expect to be able to replicate their results, as minor deviations from the data and analytics procedures of the original authors are likely to lead to null results.
...
Combining the evidence above with the concerns raised by @Ohlson:2022aa and it seems difficult to distinguish much of contemporaneous accounting research from what you would see if p-hacking were the modus operandi of most researchers.
...
Second, the ability to simply make up "interesting" research questions is a p-hacker's dream.^[Think "The effect of US decline in science on suicide rates" as a paper title based on the correlation from Tyler Vigen's website that I discussed above.]
...
Going first, researchers such as @Ball:1968ub and @Beaver:1968vf could pick the "low-hanging fruit" and address fundamental questions of the discipline, to test alternative hypotheses that were more likely to be true and easier to demonstrate empirically.
...
And, as discussed above, a typical replication conducted without some cooperation from the original authors is likely to yield difference in results that are difficult to explain, requiring exhaustive checks and iterations to understand them.
...
And as we have discussed, if the results have been p-hacked, sharing code and data dramatically lowers the cost of showing how fragile the results are, creating an incentive to make replication difficult.
...
We offer initial attempt to incorporate this topic into a PhD curriculum in [our course book](https://iangow.github.io/far_book/natural-revisited.html) and hope that others find ways to incorporate the topic into the PhD curriculum so as to raise awareness of the issue.
...
@Gow:2016kn [p. 499] argue that "accounting research can benefit substantially from more in-depth descriptive research" and that "this type of research is essential to improve our understanding of causal mechanisms and [to] develop structural models."
...
# The other elephants {#sec-other-elephants}

Above I explained that the basic Elephant #5 of @Ohlson:2022aa ("Issues related to 'screen-picking' and 'data-snooping'") is synonymous with p-hacking.
...
Elephant #1 ("Referring to the absence of a Fama-MacBeth analysis") is likely to be seen when researchers are reluctant adjust their standard errors in ways that make results disappear.

Elephant #2 ("Asking whether a key RHS variable contributes to explaining the dependent variable") and #3 ("It takes more than stars to settle the matter") both raise uncomfortable questions when the results are p-hacked, as the explanatory value of the independent variable is likely to be low and the economic significance of any apparent relation is likely to be small when the sample is large.
