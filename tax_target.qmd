...
In the code below, I generate data for 1,000 firms and draw ETRs for Q3 and Q4 from independently from a uniform distribution over the range 33%--37%.
...
```{r}
ebt_forecast <- rnorm(n = n, mean = 1, sd = 0.2)
...
```{r}
ebt_actual <- ebt_forecast + rnorm(n = n, sd = 0.1)
ni_actual <- ni_forecast * (1 - etr_4)
```
...
```{r}
df <- 
  tibble(etr_3, etr_4, ebt_actual, ni_forecast, ni_actual) |>
...
```{r}
fms <- list(
  lm(etr_4 - etr_3 ~ prebeat_amt, data = df, subset = prebeat),
  lm(etr_4 - etr_3 ~ premiss_amt, data = df, subset = premiss)
)
```
