...
In writing this note, I use several packages including those listed below.^[Execute `install.packages(c("tidyverse", "DBI", "duckdb", "farr", "arrow", "dbplyr")` within R to install all the packages you need to run the code in this note.]
...
The source is "the [U.S. Bureau of Labor Statistics (BLS) website](https://www.bls.gov/audience/economists.htm)" and the authors say "we downloaded Zip files. We then merged these data with our sample dataset. There was no manual entry at any point in the process."
...
I tend to use an environment variable (`RAW_DATA_DIR`) to indicate where I put "raw" data files like these and in this case, I put them in a directory named `bls` under that directory.
...
In this case, I will put the data under `bls`.
...
```{r}
#| label: get_bls_data_apply
#| cache: true
#| dependson: years, get_bls_data
system.time(lapply(years, get_bls_data))
```

My next approach does all the processing using DuckDB.
I unzip the data, then use DuckDB's `read_csv()` piped into a direct save to parquet files, again saved in the same location.^[So the files created in the previous step will be overwritten here.]

```{r}
#| label: get_bls_data_duckdb
#| cache: true
...
```{r}
#| label: get_bls_data_duckdb_apply
#| cache: true
#| dependson: years, get_bls_data_duckdb
...
Second, I use `load_parquet()` from the `farr` package with a wildcard (`"bls_all_*"`) to load *all* the BLS data into a single table.
...
bls_data
```
I next make a nifty function that takes a lazy dataframe inside DuckDB (such as `bls_data`) and saves it as a parquet file inside the data repository used to store the 
parquet files above.
...
For example, all the BLS data created using my version of the code can be found [here](https://www.dropbox.com/scl/fo/g5yat3ugd6n95fl0y71kp/AHX6QGGxWA6-Bid69a-Ys1Q?rlkey=dgzcgfdq7h7by0b1qpxwgedce&dl=0).^[Note that this is as far as I got in looking at the reproducibility of the steps in the original paper.]
