...
One thing that BKLYZ1 is very clear on is that the test period is 2003--2011.
...
Yet this is simply an impossible reading of BKLYZ1, where 2003--2011 is clearly the test period.
...
But relabeling "serial frauds" also does not make sense in that it is recoding a fraud as 1 in 2003 so that a "prediction" of the same underlying fraud can be made in 2005 even though, by the terms of the example itself (Figure 1 of BKLYZ), the SEC does not release an AAER until 2007.

That one cannot be training models using data of frauds that have not yet been released to predicting frauds in the test period that also affect firm-years in the training period is clearly not appropriate.

It seems clear from BKLYZ1's discussion of "serial fraud" that the authors were well aware of the issues here.
...
One response the authors might have is that there might be a "key event [that] reveals 2001-2003 fraud labels" before 2004.
But this describes a completely different prediction problem from that studied in BKLYZ1 (or BKLYZ0), which identifies frauds as AAER events.
...
This is a completely different prediction problem (and perhaps the one embedded in BKLYZ0, which appears to use information about serial frauds that will be revealed after the test period in developing a training model using those very same frauds).
...
One possible response by the BKLYZ team might be that they have more than complied with the JAR data policy in effect when they submitted there paper and therefore do not need to account for the "coding error" beyond the code they have already provided.
I think this kind of response would be helpful to their case.
