...
`$DATA_DIR/edgar/filings_expanded.parquet`. The repository `.env` supplies those
...
```sh
.venv/bin/python scripts/extract_filings.py /path/submissions.zip /path/expanded.parquet --max-seconds 0
```
...
| File | Contents |
| --- | --- |
| `filings_expanded.parquet` | All fields from recent and referenced historical filing arrays, plus `cik` and `source_file` |
| `filings_expanded_companies.parquet` | One row per primary company JSON, with company-level fields including `ownerOrg` |
| `filings_expanded_addresses.parquet` | Addresses by `cik` and `address_type` |
| `filings_expanded_tickers.parquet` | Ticker/exchange pairs by array position, padding unequal arrays with nulls |
| `filings_expanded_former_names.parquet` | Former names and their original `from`/`to` timestamp text |
| `filings_expanded_files.parquet` | Historical-file references, counts, date ranges, and an `available` flag indicating ZIP membership |
...
are integers; sizes and counts are big integers; indicator fields are Booleans.
...
A sibling `filings_expanded.staging` directory holds JSON Lines files and a
