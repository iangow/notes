    ...
    def test_tables_and_interrupted_resume(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with duckdb.connect() as con:
                ...
                self.assertEqual(paths['companies'], root / 'companies.parquet')
                self.assertEqual(paths['tickers'], root / 'tickers.parquet')
                ...
                former_date, former_date_type = con.execute("SELECT \"from\", typeof(\"from\") FROM read_parquet(?)", [str(paths['former_names'])]).fetchone()
                self.assertEqual(str(former_date), '2000-01-01')
                self.assertEqual(former_date_type, 'DATE')
