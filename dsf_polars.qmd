...
However, some of this expressiveness relies on data being in forms (for example, with dates ready to make an index) that often differ from the data we have, and pandas can struggle to manipulate the data into those forms, especially with larger data sets.
...
This variant may have particular appeal to pandas experts because the code is likely more familiar to experienced analysts of data frames.
...
Looking at @fig-ibis-pg, it seems that this code also works to reproduce the data frame created by the pandas code in @sec-pandas.
