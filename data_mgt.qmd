...
Within Python, you can check the version using the following.^[This will not work if you have an older version of `db2pq`, in which case you should reinstall it.]
...
I can use the function `pq_restore()` to make the one from `2025-11-09` the one that I am using for my data repository.
...
If we update again with `archive=True`, we will effectively put the `2025-11-09` back in the archive and replace it with the current version on WRDS.
...
To examine this issue, I'm going to look revisit the Call Report data I wrote about [recently](https://rpubs.com/iangow/call_reports).
...
```
WRDS generally stores timestamps in PostgreSQL with type `TIMESTAMP WITHOUT TIME ZONE`, which is equivalent to saying "you figure out the time zone, user."^[The only option used by PostgreSQL should `TIMESTAMP WITH TIME ZONE`.]
...
```{python}
rcfa_1 = load_parquet("wrds_call_rcfa_1", "bank", data_dir = "temp")
(
    rcfa_1
    .select("rssd9001", "wrdsreportdate", "rssdsubmissiondate")
    .filter(pl.col("rssd9001") == 37, 
