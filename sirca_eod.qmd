...
SIRCA's ASX end of day price collection includes the tables listed in @tbl-asx-eod-keys.
...
I merely assume that you have access to SIRCA and downloaded the raw data from SIRCA along the lines discussed below.
...
Because we specified `compute(name = "si_au_ref_names")`, we can refer to that table in SQL such as the following:
...
Again the same query could be run using `tidyverse` code.
...
tdays_elapsed_df <-
  ...
  mutate(tdays_elapsed = td - lag(td),
         lag_date = lag(date)) |>
  select(gcode, date, lag_date, tdays_elapsed) |>
  ungroup()
```
