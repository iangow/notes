...
adj_rets <-
  ...
  mutate(CorporateOnlyAdjustedPrice = corporatefactor * close,
...
anz_cum <-
  adj_rets |>
  filter(gcode == "anz1") |>
  select(gcode, date, CorporateOnlyAdjustedPrice, AdjustedPrice, close) |>
  compute()
```
...
anz_cum |>
  group_by(gcode) |>
  window_order(date) |>
  mutate(across(c(AdjustedPrice, CorporateOnlyAdjustedPrice),
                \(x) coalesce(x / lag(x), 1)),
         across(c(AdjustedPrice, CorporateOnlyAdjustedPrice),
                \(x) exp(cumsum(log(x))))) |>
  window_order() |>
  pivot_longer(cols = ends_with("Price"), 
               names_to = "series", values_to = "price") |>
