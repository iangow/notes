...
Notice the `seniorsecurity` field distinguishes these two series.
...
@tbl-neg-cum-factors shows that there are actually some cases where `factor == -1`, but `cumulativefactor` is not negative.
...
The following query confirms that when `factor == -1` and `cumulativefactor != -1`, there is no value of `close`.
...
Table @tbl-wsf1-factor shows another example with a dividend on `2004-07-05`.
...
#| tbl-cap: "Calculating `cumulativefactor`"
#| render: !expr function(x, ...) knit_print_alt(x, ...)
si_au_prc_daily |>
  group_by(gcode, seniorsecurity) |>
  window_order(desc(date)) |>
