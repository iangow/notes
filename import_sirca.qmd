...
In the following code snippet, we convert the numeric variables to the types we determined to be appropriate throught the analysis above.
...
... and character vectors 14 through 20.
...
This has all the hallmarks of someone importing data into Microsoft Excel as part of their process.
...
To see why underscores are used, we can remove the underscore and save the `Gcodes` in a CSV file.
...
To be frank, I struggle to see any reason why Excel should have any part in the data science workflow.
...
Looking at `ListDate_YMD` and `ListDate` are always equal.
...
```
So we have two version of the date variables related to listing dates (`ListDate_YMD` and `ListDate`) and delisting dates (`DelistDate_YMD` and `DelistDate`) and perhaps it makes sense to keep just one of each.
If the values in each of the pair is the same as the other, then there's no reason to 
...
Specifically, we should choose whichever of `DelistDate_YMD` and `DelistDate` that is consisten with `DelistDate_DaysSince`.
...
```
If we take the row with non-`NA` values for `GICSIndustry` and `SIRCAIndustryClassCode` to be the correct one, then we need to delete the other row.
