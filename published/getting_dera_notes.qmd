...
In a [recent note](missing_form_aps.html), I used XBRL data to identify potentially missing Form AP filings.
...
This note aims to provide this missing information.^[Guidance on downloading the Form AP data is provided in [an earlier note](../form_ap_names.html) I wrote.]
...
The source code for this note is available [here](https://raw.githubusercontent.com/iangow/notes/main/published/getting_dera_notes.qmd).

This note uses the following R packages:^[To install these packages, run `install.packages(c(tidyverse, "DBI", "farr", "httr2", "rvest", "arrow")` in the console of RStudio. For current DERA downloading code, install the `dera.pq` package using `pak::pak("iangow/dera_pq")`.]
...
For the task considered in the [note discussed above](missing_form_aps.html), we needed the *Financial Statement and Notes* data set, so I focus on that data set here.
...
The standard tags are derived from taxonomies in the SEC's [standard taxonomies file](https://www.sec.gov/info/edgar/edgartaxonomies.shtml) as of the date of submission.
...
fields for the group and the arc.^[The current code I use to download and update these data is in the [`dera.pq`](https://github.com/iangow/dera_pq) package. Run `dera.pq::update_dera_notes()` to get these data.]
...
![Financial Statement and Notes website](images/dera_notes.png){#fig-dera-notes style="max-width: 560px; width: 100%; height: auto;"}
...
![Contents of `2024_10_notes.zip`](images/2024_10_notes.png){#fig-example style="max-width: 480px; width: 100%; height: auto;"}
...
The code I now use for this is packaged in [`dera.pq`](https://github.com/iangow/dera_pq).
The package provides functions to list available SEC files, download and parse each archive, write the component tables as Parquet files, and store source metadata in the Parquet files.
...
The function `available_dera_notes()` can be used to get the list of `.zip` files shown on the SEC website.

```{r}
zip_files <- dera.pq::available_dera_notes(user_agent = getOption("HTTPUserAgent"))
zip_files
```

The high-level update function downloads only missing or changed source files and writes the Parquet files under `$DATA_DIR/dera_notes`.

```{r}
#| eval: false
dera.pq::update_dera_notes()
```
...
In the past I have used three different approaches to this kind of problem:
...
The `dera.pq` package uses the first approach.
For each SEC source archive, it writes the raw `Last-Modified` header and a normalized `last_modified_utc` value into the schema metadata of each output Parquet file.
On later runs, `update_dera_notes()` compares the current SEC metadata with the metadata embedded in the local Parquet files and skips source files that are already current.
