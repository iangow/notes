...
```{r}
#| include: false
pq_archive("rpa_entity_mappings", "ravenpack_common", data_dir = "data")
```
...
In such cases, it may be useful to use SAS data to determine the vintage of the data.
However, as noted above, SAS-related functionality (i.e., `use_sas`) has not yet been implemented in the R package `db2pq`.
...
To examine this issue, I'm going to revisit the Call Report data I wrote about [recently](https://rpubs.com/iangow/call_reports).
I have these data in my (general-purpose) data repository and I can use the following code to load it into R using `arrow`.
...
```{r}
#| include: false
pq_archive("wrds_call_rcfa_1", "bank", data_dir = "data")
```
...
WRDS generally stores timestamps in PostgreSQL with type `TIMESTAMP WITHOUT TIME ZONE`, which is equivalent to saying "you figure out the time zone, user."^[The only option used by PostgreSQL should be `TIMESTAMP WITH TIME ZONE`.]
