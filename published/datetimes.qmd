...
::: {.abstract}
The purpose of this note is address the topic of temporal data (dates and times) in more detail than found in the ["Dates and Times" chapter](https://r4ds.hadley.nz/datetimes) of *R for Data Science* [@Wickham:2023aa] and to provide a hands-on application of working with them.
The application I work with is based on the SEC submissions data I discussed in [another recent note](sec_submissions.html).
I study this application using R and DuckDB, as moving data between systems can be a pain point, especially with date-times.
:::

As discussed in a [recent post on LinkedIn](https://www.linkedin.com/posts/iangow_working-with-json-data-the-case-of-sec-submissions-activity-7199520005409906690-XlFZ), one goal of [*Empirical Research in Accounting: Tools and Methods*](https://iangow.github.io/far_book/) [@gow2024empirical] is to provide a pathway to mastery of the contents of @Wickham:2023aa.^[@gow2024empirical was published in print form by CRC Press in December 2024 and remains free online.]
That said, I identified a few gaps, including spreadsheets, hierarchical data, and dates and times.
[One recent note](sec_submissions.html) covered hierarchical data and [another note](spreadsheets.qmd) addresses spreadsheets.

The purpose of this note is address the gap regarding dates and times in more detail---including a hands-on application---than found in the ["Dates and Times" chapter](https://r4ds.hadley.nz/datetimes) of @Wickham:2023aa.
The application I work with is based on the SEC submissions data I discussed in [another recent note](sec_submissions.html).
I study this application using R and DuckDB, as moving data between systems can be a pain point, especially with date-times.
...
The source code for this note is available [here](https://raw.githubusercontent.com/iangow/notes/main/published/datetimes.qmd).
...
[Chapter 17](https://r4ds.hadley.nz/datetimes) of @Wickham:2023aa identifies three data types related to dates and times:
...
Like *R for Data Science*, I concentrate on dates and date-times.^[It is pointed out in that R does not have a native class for storing times [@Wickham:2023aa, p.296].]
...
We start by loading the `filings` data stored in a parquet file `filings.parquet` in the `edgar` schema of our data repository.^[The code below assumes that `DATA_DIR` is set in the project `.env` file and points to the root of that repository.
The process for creating this file is described [here](sec_submissions.html).
You can obtain a copy of `filings.parquet` [here](https://www.dropbox.com/scl/fi/9osie7ln50z67uzi9zys5/filings.parquet?rlkey=5igrlb23tns4n8trunk3nstlt&dl=0).
If you do not use a Parquet data repository, place `filings.parquet` in the `data` subdirectory of this project and use `filings <- load_parquet(db, table = "filings", data_dir = "data")` instead.]
Because the SEC bulk submissions file is updated over time, the results below may differ from those in earlier rendered versions of this note.

```{r}
filings <- load_parquet(db, table = "filings", schema = "edgar")
```
...
However, the bulk JSON contains a mixture of UTC and `America/New_York` clock values despite this suffix.
The current Parquet file reconciles timestamps using comparisons across snapshots, duplicate accessions, live SEC JSON, and selected SGML headers; it also uses file-level timezone inference where the available evidence agrees.
Rows without supporting evidence retain an explicitly labelled Eastern-time fallback, rather than being treated as verified.
The file includes `timestamp_provenance` and `timestamp_interpretation` columns so that these cases can be distinguished.^[The correction workflow is documented [here](https://github.com/iangow/notes/blob/main/scripts/TIMESTAMP_WORKFLOW.md). The timestamps are stored as timezone-aware instants in Parquet and displayed here in New York time.]
...
filing_times <- 
  ...
  select(cik, year, accessionNumber, acceptanceDateTime, acceptance_time,
         form, hour) |>
  compute(name = "filing_times", overwrite = TRUE)
```
...
filing_times |>
  count(acceptance_time) |>
  collect() |>
  ggplot(aes(x = acceptance_time, weight = n)) +
...
filing_times |>
  ...
  count(year, midnight) |>
  collect() |>
  ggplot(aes(x = year, y = n, fill = midnight)) +
  geom_col() +
  theme(axis.text.x = element_text(angle = 90))
```
...
filing_times |> 
  ...
  mutate(year = as.character(year)) |>
  count(acceptance_time, year) |>
  collect() |>
  ggplot(aes(x = acceptance_time, fill = year, weight = n)) +
  geom_histogram(binwidth = 5 * 60) +
  scale_x_datetime(date_breaks = "1 hour",
                   date_labels = "%H:%M") +
  theme(axis.text.x = element_text(angle = 90))
...
@fig-time-later-timed focuses on post-2001 non-midnight filings, where we see that the bulk of filings are between 06:00 and 22:00, consistent with the statement on [the SEC website](https://www.sec.gov/submit-filings) that "EDGAR is available to accept filings from 6 a.m. to 10 p.m. ET weekdays (except federal holidays)."
These hours provide a useful diagnostic for timestamp problems, but they do not establish the timezone of an individual JSON clock.
A UTC clock can fall within this window, and some genuinely Eastern timestamps fall outside it, so corrections require additional evidence.

However, some pre-08:00 filings appear in @fig-time-later-timed.
Filings accepted outside normal EDGAR hours---that is, after 22:00 and before 06:00---are not just a historical artifact.
@fig-outside-hours shows outside-hours filings from 2003 onward as a percentage of all filings in each year, split by form category.
The correspondence category combines `CORRESP`, `UPLOAD`, and `DRSLTR`.
The concentration at 00:15 is largely driven by `EFFECT` filings, which suggests that at least some of these times reflect SEC processing rather than ordinary filer submission behavior.

```{r}
#| label: fig-outside-hours
#| fig-cap: "Recent outside-hours filings by form category"
year_totals <-
  filing_times |>
  filter(year >= 2003) |>
  count(year, name = "year_filings") |>
  collect() |>
  mutate(year = as.integer(year))

filing_times |>
  filter(year >= 2003,
         acceptance_time > "2000-01-01 22:00:00" |
           acceptance_time < "2000-01-01 06:00:00") |>
  mutate(filing_category = case_when(
    form == "EFFECT" ~ "EFFECT",
    form %in% c("CORRESP", "UPLOAD", "DRSLTR") ~ "Correspondence",
    TRUE ~ "Other"
  )) |>
  count(year, filing_category) |>
  collect() |>
  mutate(year = as.integer(year)) |>
  left_join(year_totals, by = "year") |>
  mutate(filing_pct = n / year_filings,
         filing_category = factor(
           filing_category,
           levels = c("EFFECT", "Correspondence", "Other")
         )) |>
  ggplot(aes(x = year, y = filing_pct, fill = filing_category)) +
  geom_col() +
  scale_x_continuous(breaks = seq(2003, 2026, by = 2)) +
...
#| fig-cap: "Distribution of acceptance time--post-2001 with times"
filing_times |>
  filter(year >= 2002,
         acceptance_time > "2000-01-01") |>
  count(acceptance_time) |>
...
@fig-early-morning zooms in on the earliest of the pre-08:00 filings that appear in @fig-time-later-timed, focusing on filings from just after midnight through 01:00.
It appears that these are oddly concentrated at around 00:15 and are perhaps data errors.

```{r}
#| label: fig-early-morning
#| fig-cap: "Distribution of acceptance time--outliers before 01:00"
filing_times |>
...
filing_times |>
  ...
  mutate(month = floor_date(acceptanceDateTime, "month")) |>
  count(month) |>
...
sql <- dbplyr::sql(str_c("SELECT * FROM read_csv('", url, "', ",
...
We can get the `tickers` file from Dropbox using the code below.^[See [the note](sec_submissions.html) discussed earlier for details on how this file was created from data on SEC EDGAR.]

```{r}
url <- str_c("https://www.dropbox.com/scl/fi/lertyk4twnphum0g8wrjv/",
             "tickers.parquet?rlkey=mwm0m2m9c0ylg5h6yg1ivr38p&dl=1")
sql <- str_c("read_parquet('", url, "')")
tickers <- 
  tbl(db, dbplyr::sql(str_c("SELECT * FROM ", sql))) |>
