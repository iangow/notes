...
[One recent note](sec_submissions.html) covered hierarchical data and a forthcoming note will address spreadsheets.
...
You can obtain a copy of `filings.parquet` [here](https://www.dropbox.com/scl/fi/6pdu8ynwkmhg26zc1ekc9/filings.parquet?rlkey=nccfjunomj2l7lqd3clvqwk9u&dl=0).
...
Many of these outside-hours observations are ownership filings, especially Form 4 filings, or `EFFECT` filings.
...
filing_times |>
  ...
  mutate(filing_category = case_when(
    form == "EFFECT" ~ "EFFECT",
    form %in% c("3", "3/A", "4", "4/A", "5", "5/A") ~ "Ownership",
    TRUE ~ "Other"
  )) |>
  ...
  mutate(filing_pct = n / year_filings,
         filing_category = factor(
           filing_category,
           levels = c("Ownership", "EFFECT", "Other")
         )) |>
...
```{r}
#| results: false
dbExecute(db, "ATTACH '' AS pg (TYPE POSTGRES, READ_WRITE)")
```
...
I use `CREATE OR REPLACE` so that the PostgreSQL table reflects the same underlying data as the Parquet file.

```{r}
#| include: false
data_dir <- Sys.getenv("DATA_DIR")
sql <- str_glue("CREATE OR REPLACE TABLE pg.filings AS ",
                "SELECT * FROM '{data_dir}/edgar/filings.parquet'")
...
```{r}
#| eval: false
sql <- "CREATE OR REPLACE TABLE pg.filings AS
...
```{r}
#| results: false
dbExecute(db, "CREATE OR REPLACE TABLE pg.tickers AS
               SELECT * FROM tickers")
dbExecute(db, "CREATE OR REPLACE TABLE pg.sample_calls AS
...
The following query converts `hour` and `minute` to integers, as the `make_timestamptz()` function in PostgreSQL expects integers in those slots.
...
The code for creating `earnings_filings` is unchanged from above, as PostgreSQL offers the same `regexp_split_to_table()` function that we used in DuckDB.
...
The code creating `sample_merged` differs slightly because `start_date - acceptanceDateTime` results in a column of type `interval` in PostgreSQL that R is unable to interpret easily.
