...
categories: ["Data curation", Polars, DuckDB]
format:
  html:
    toc: true
    toc-location: right
...
The FFIEC data sets are not as amenable to automated downloading as those offered by other government agencies such as the SEC (see my earlier [note on XBRL data](getting_dera_notes.html)), the PCAOB (see my [note on Form AP data](../form_ap_names.html)), or even the Federal Reserve itself (I used data from the [MDRM site](https://www.federalreserve.gov/apps/mdrm/) in preparing this note).
...
Having set these environment variables, I can load my package and run a single command `ffiec_process()` without any arguments to process *all* the raw data files.^[Note the using the argument `use_multicore = TRUE` gives me about a five-time improvement in performance, but you will see different results depending on your system.
...
#| eval: false
#| output: true
results <- ffiec_process(use_multicore = FALSE) |> system_time()
```
