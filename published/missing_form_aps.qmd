...
categories: [Research methods, PCAOB, XBRL, SEC]
keywords:
  - PCAOB
  - Form AP
  - XBRL
...
For the task we consider here, we need to use the *Financial Statement and Notes* data set.^[The current code I use to download and update these data is in the [`dera.pq`](https://github.com/iangow/dera_pq) package. Run `dera.pq::update_dera_notes()` to get these data.]
...
The source code for this note is available [here](https://raw.githubusercontent.com/iangow/notes/main/published/missing_form_aps.qmd) and the latest version of this PDF is [here](https://raw.githubusercontent.com/iangow/notes/main/published/missing_form_aps.pdf).
...
auditor_names <-
  form_aps |>
  select(firm_id, firm_name, audit_report_date) |>
  group_by(firm_id) |>
