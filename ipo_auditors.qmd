...
In terms of @fig-whole-game, @Broman:2018aa is largely about "Collect" and @Wickham:2023aa picks up with "Import".
...
For all the problems, using spreadsheets removes the need for such skills.
...
All these issues are even worse with Microsoft Excel because there is no "track changes" or "compare documents" feature to assist with clean-up of messes from Bob-or-Anne-edited-the-wrong-version situations.
...
With Google Sheets, it is easy to add data validation on the fly is easy.
...
Almost everyoine has a Google ID and this can be used to grant access.
...
## Combining data to produce link table
...
### Identifying more problematic links
...
all_matches |>
  ...
  arrange(dealID) |>
  collect() |>
  write_sheet(ss = gs_target, sheet = "all_auditors")
```
...
I do not disagree with this one, but I would say that it can be fine to use font colour or highlighting to communicate with RAs: "Please focus on the cells I have coloured green before moving on to the other cases."
...
This recommendation [@Broman:2018aa, p. 8] perhaps has not aged well.
...
I would need to export data to a different format (e.g., Microsoft Excel) and back *that* up if my concern is that Google might disappear taking my data with it.
...
Nor is it clear that a back-up between each row.

If you're a researcher working on papers, a more sensible view might be that some back-up be retained for each version of a paper that is posted or shared with others.
...
If the reason for restoring the data is some issue that came up in the last week of 2024 and I'd prefer the `glucose_6_weeks_2024-12-21` version.
...
My recommendation is to rely on version control features embedded in Google Sheets.
...
In short, I am not sure that I fundamentally disagree with this recommendation if it simply means that there should be a "persist" step along the lines of that depicted in @fig-whole-game.
