...
In a nutshell, we have processed each of the 99 zip files into six files.
...
From the output above, we see that item `RIAD4230` appears on both Schedule RI (`ri`) and Schedule RI-BII (`ribii`).
The first question might be: What is `RIAD4230`?
We can get the answer from `ffiec_items`:
...
Surprisingly, there are very few data items related to dates in the Call Report data.
...
ffiec_str |>
  inner_join(ffiec_items, by = join_by(item), copy = TRUE) |>
  count(mnemonic, sort = TRUE) |>
  collect()
```

### Schedules data

The final set of tables contain information abot th

One field that warrants explanation is `schedule`, which is the list of schedules from which the data item in question was taken.
As some values appear on multiple schedules even in a given quarter, `schedule` may contain more than one item.

```{r}
ffiec_schedules <- ffiec_scan_pqs(db, "ffiec_schedules")

ffiec_schedules |>
  mutate(n_schedules = length(schedule)) |>
  filter(n_schedules > 1) |>
...
Working with dates and times (**temporal data**) can be a lot more complicated than in generally appreciated.
...
I chose to use parquet files for `ffiec.pq`.
...
In general, I think one wants to be fairly conservative in considering database principles with a data package.
...
For all of the schedules, `IDRSSD` and `date` combine to make a valid primary key.
To facilitate checking this, I include the function `ffiec_check_pq_keys()`, which checks that a proposed primary key valid for the table formed by combining the associated files for all quarters for a specified `schedule`.
I first apply to it the Schedule RC table (`schedule = "rc"`):


We can see that `c("IDRSSD", "date")` is a valid primary key for Schedule RC because there are no rows in `null_violations` and no rows in `pk_violations` (checks for uniqueness) and `ok` is `TRUE`.

We can use `ffiec_list_pqs()` to identify all schedules and pass that to `ffiec_check_pq_keys()`:

As can be seen, `IDRSSD` and `date` form a primary key for all schedules (including the POR data).
...
Given that I am delivering the Call Report data not as the data files, but as an R package along with instructions for obtaining the zip files from the FFIEC Bulk Data website.
...
For my own purposes, I achieve a modest level of data version control by using Dropbox, which offers the ability to restore previous versions of data files.
