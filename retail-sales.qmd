...
3. In creating @tbl-reconc, I used `left_join()` and then `coalesce()`.
...
Some examples of cross-sectional data sets are:
 ...
 - Stock prices for multiple companies on a given day
 - Measures of the gross domestic product for different economics for a given quarter
...
Some examples of panel data sets are:
 ...
 - Quarterly measures of the gross domestic product for different economics over several periods
 - Annual profits for several companies over several year

## Plotting panel data

The `retail_sales` data set is a panel data set with monthly data on retail sales for several "states" and several industries.
For a given state and industry, the monthly retail sales series is a time-series.
For an initial exploration of `retail_sales` in more depth, I construct `df_group_plot`, a data set focused on a subset of the available dates and the data for all states combined (`state == "Total (State)"`) and the industry groups (`ind_level == 1`).
...
Figures [-@fig-all-ind] and [-@fig-all-ind2] plot the same underlying information in two different ways.
...
If we are in August of 2025, it does not make omparing the sales for 2025 with total sales for 2024, as this would be comparing sales for seven months (January through July) with twelve months.
So it is common to calculate **year-to-date** performance and compare that with performance for a comparable period from earlier years.
...
```{r}
fiscal_month <- function(x, first_month = 1, label = TRUE, abbr = TRUE) {
...
df_group_fytd_plot <-
  cafes_vic_wa |>
  mutate(fyear = year(date) + as.integer(month(date) >= 7)) |>
  ...
  mutate(fyear = as.character(fyear),
         month = fiscal_month(date, first_month = 7))
```
