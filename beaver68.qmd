...
Some aspects of the @Beaver:1968vf are confusing unless read closely, so we provide some explanation here.
...
#### Option 1: Use DuckDB with WRDS PostgreSQL
...
#| label: load_wrds
#| eval: true
def load_wrds(db, table, schema, wrds_id=None):
    if not wrds_id:
        wrds_id = os.getenv('WRDS_ID')
    # uri = f"postgres://{wrds_id}@wrds-pgdata.wharton.upenn.edu:9737/wrds"
    uri = f"postgres://localhost:5432/iangow"
    
    return db.read_postgres(uri, table_name=table, database=schema)
```
...
fundq = load_wrds(db, "fundq", "comp")
ccmxpf_lnkhist = load_wrds(db, "ccmxpf_lnkhist", "crsp")
dsi = load_wrds(db, "dsi", "crsp")
dsf = load_wrds(db, "dsf", "crsp")
stocknames = load_wrds(db, "stocknames", "crsp")
```

#### Option 2: Connect directly with PostgreSQL

```{python}
#| label: connect-pg
#| eval: false 
wrds_id = os.environ['WRDS_ID']
#db = ibis.postgres.connect(user=wrds_id,
#                            host="wrds-pgdata.wharton.upenn.edu",
#                            port=9737,
#                            database="wrds")
db = ibis.postgres.connect()                            
                            
fundq = db.table("fundq", "comp")
ccmxpf_lnkhist = db.table("ccmxpf_lnkhist", "crsp")
dsi = db.table("dsi", "crsp")
dsf = db.table("dsf", "crsp")
stocknames = db.table("stocknames", "crsp")
```

#### Option 3: Use DuckDB with local parquet repository
...
(In this case, we use the `seq` function to create a list of *all* dates over the period represented on `crsp.dsi`.)
...
3. Fill in the missing `td` entries by grabbing the next available `td`. (For this, we use the `fill()` function from the `tidyr` package.)

```{python}
#| label: annc_dates
min_date = (
    trading_dates
    .aggregate(date = _.date.min())
    .to_pandas()
    .date[0])

max_date = (
    trading_dates
    .aggregate(date = _.date.max() + ibis.interval(days=1))
    .to_pandas()
    .date[0])

annc_dates_pd = db.sql("SELECT generate_series::date AS annc_date" 
       f" FROM generate_series(DATE '{min_date}',"
       f" DATE '{max_date}', INTERVAL '1' DAY)")
