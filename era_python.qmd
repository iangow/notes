...
As discussed below, I think it would be possible to do this without losing the elegance and efficiency of the current approach using R.
...
Suppressing some messy details, translation of either *Empirical Research in Accounting: Tools and Methods* or *Tidy Finance with R* from R to Python using standard tools like SQLAchemy and Pandas involves two steps.
...
So this approach with a Python translation would mean that SQL strings appeared throughout the book.
...
For example, the code [here](https://iangow.github.io/far_book/rdd.html#bloomfield2021va) that creates `risk_asymmetry` takes about one second to run, but would take many minutes using an approach that loaded data into a data frame and ran regressions in R or Python.^[Earlier versions of the code in fact took many minutes to run.]
...
The [code I chose](https://iangow.github.io/far_book/parquet-wrds.html#working-with-parquet-larger-data-sets) for translation uses `crsp.dsf`, a relatively large table that illustrates nicely the benefits of working with remote data.
...
This will allow use to seem an important difference between Ibis/Pandas and `dbplyr`/`dplyr`.
