---
title: "Beaver (1968): Python version using WRDS PostgreSQL"
author: 
  - name: Ian D. Gow
...
This note uses the WRDS PostgreSQL database.
...
The source code for this note is available [here](https://raw.githubusercontent.com/iangow/notes/main/beaver68-pg.qmd) and the latest version of this PDF is [here](https://raw.githubusercontent.com/iangow/notes/main/beaver68-pg.pdf).
:::
...
I have the my WRDS ID set in the environment variable `WRDS_ID`.
To run this code, you should either do the same or replace the value of `user` in the code below with your WRDS ID. 
...
def load_wrds(db, table, schema):
    return db.table(table, database=("wrds", schema)) 
```
...
con = db.raw_sql(f"ATTACH '{uri}' AS wrds (TYPE postgres)")
```
...
mkt_rets = (
    dsf 
    .mutate(ret = _.ret.cast("float64"),
            vol = _.vol.cast("float64"))
    .inner_join(dsi, "date")
    .mutate(vwretd = _.vwretd.cast("float64"))
