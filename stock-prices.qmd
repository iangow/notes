...
dmp_ret_data <-
  ...
  group_by(symbol) |>
  arrange(date) |>
  mutate(close_open_amt =lag(close) - (open + dividend),
...
```{r}
dmp_ret_data |>
  filter(dividend != 0, !is.na(close_open_amt)) |>
  select(symbol, date, close_open_amt, dividend) |>
  ggplot(aes(x = dividend, y = close_open_amt)) +
  geom_point() +
